        ...
        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
            ...
            <div>
                ...
                <p class="text-gray-500 text-xs sm:text-sm mt-1">
                    Mise à jour des informations du militaire
                </p>
            </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6">
                    ...
                    <div>
                        ...
                        <select name="id_grade"
                            class="w-full border rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#4B0082]"
                            required>
                        ...
                        <select name="id_service"
                            ...
                            @foreach ($services as $service)
                                <option value="{{ $service->id_service }}"
                                    {{ $personnel->id_service == $service->id_service ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="id_service"
                            class="w-full border rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#4B0082]"
                            required @if (Auth::user()->type != 'admin') disabled @endif>
                            ...
                            @foreach ($services as $service)
                                @if (Auth::user()->type == 'admin' || Auth::user()->personnel->id_service == $service->id_service)
                                    <option value="{{ $service->id_service }}"
                                        {{ $personnel->id_service == $service->id_service ? 'selected' : (Auth::user()->personnel->id_service == $service->id_service ? 'selected' : '') }}>
                                        {{ $service->nom_service }}
                                    </option>
                                @endif
                            @endforeach
                        ...
                        @if (Auth::user()->type != 'admin')
                            <!-- si disabled, envoyer quand même la valeur -->
                            <input type="hidden" name="id_service" value="{{ $personnel->id_service }}">
                        @endif
                        ...
                        <select name="id_fonction"
                            ...
                            @foreach ($fonctions as $fonction)
                                <option value="{{ $fonction->id_fonction }}"
                                    {{ $personnel->id_fonction == $fonction->id_fonction ? 'selected' : '' }}>
